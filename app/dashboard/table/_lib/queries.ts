...
export async function getContacts(params?: GetContactsParams): Promise<{
  ...
  let query = supabase.from("registry_contacts").select("*", { count: "exact" })
  ...
  if (params?.columnFilters && params.columnFilters.length > 0) {
    params.columnFilters.forEach((filter) => {
      ...
      const filterValue = typeof filter.value === "object" && filter.value !== null && !Array.isArray(filter.value)
        ? (filter.value as { operator?: string; value?: unknown }).value
        : filter.value
      
      const operator = typeof filter.value === "object" && filter.value !== null && !Array.isArray(filter.value)
      ...
      switch (operator) {
        ...
        case "isBetween":
          ...
          if (Array.isArray(filterValue) && filterValue.length === 2) {
            query = query.gte(dbColumnName, filterValue[0]).lte(dbColumnName, filterValue[1])
          }
          break
        case "inArray":
  ...
  if (params?.sorting && params.sorting.length > 0) {
    params.sorting.forEach((sort, index) => {
      ...
      if (index === 0) {
        ...
        query = query.order(dbColumnName, { ascending: !sort.desc, nullsFirst: false })
      }
    })
  } else {
    // Default sorting
