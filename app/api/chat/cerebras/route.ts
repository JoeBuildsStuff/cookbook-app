...
function convertToolsToCerebrasFormat(
  tools: Array<{
    name: string;
    description?: string;
...
export async function POST(
  request: NextRequest
): Promise<NextResponse<CerebrasAPIResponse>> {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const maxCompletionTokens = parseInt(
        (formData.get("max_completion_tokens") as string) || "65536"
      );
      const temperature = parseFloat(
        (formData.get("temperature") as string) || "1"
      );
    ...
    if (error instanceof Error) {
      if (error.message.includes("CEREBRAS_API_KEY")) {
        return NextResponse.json(
          {
            message:
              "Cerebras service is not configured. Please check the API key.",
          },
          { status: 500 }
        );
      }
...
If a tool responds with a url to a record, include it in your response using markdown.`;
    ...
    if (finalResponse) {
      ...
      const content =
        typedResponse.choices?.[0]?.message?.content || "No response generated";
